...
from .dtx_msg import DTXMessage, MessageAux, dtx_message_header, object_to_aux, dtx_message_payload_header, object_to_bytes
...
class DTXClient:
    ...
    def recv(self, client, length) -> bytes:
        ...
        while len(buf) < length:
            ...
            buf.extend(chunk)
        logging.debug(f'接收 DTX buf: {buf}')
        return buf

    def send_dtx(self, client, dtx):
        buffer = bytes(dtx)
        logging.debug(f'发送 DTX: {buffer}')
...
class DTXServer:
    ...
    def call(self, channel: Union[int, str], selector: str, *auxiliaries):
        channel_id = self.make_channel(channel)
        ret = self._call(True, channel_id, selector, *auxiliaries)
        return ret

    def send_dtx_message(self, channel_id, message_id, flags, body, expects_reply=False):
        body = object_to_bytes(body)
        pheader = dtx_message_payload_header.build({
            'flags': flags,
            'aux_length': 0,
            'total_length': len(body),
        })
        payload = pheader + body

        mheader = dtx_message_header.build({
        ...
        data.extend(mheader)
        data.extend(payload)
        self._client.send_dtx(self._cli, data)
